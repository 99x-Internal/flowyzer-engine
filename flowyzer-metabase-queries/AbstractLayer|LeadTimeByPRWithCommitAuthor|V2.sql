...
lead_time_by_pr AS (
    SELECT
        ...
        pr."stateDetail",
        pr."stateCategory",
        pr_commit.author AS "commitAuthor",
        (CASE WHEN "user".name IS NULL THEN pr_commit.author ELSE "user".name END) AS "commitAuthorName",
        task.id AS "taskId",
        ...
        sprint."startedAt" AS "SprintStarted_Time",
    ...
    JOIN "tms_Sprint" AS sprint
        ON sprint.id=task.sprint
    JOIN (
        SELECT
            LOWER(vc.author) as "author", prc."pullRequest"
    ...
    LEFT JOIN (
        ...
        ON LOWER("user".id)=pr_commit.author
    WHERE "statusChangeNewValue" = {{devInprogressStatus}}
    ORDER BY pr.id
)
SELECT
    *,
    EXTRACT(EPOCH FROM ("SprintStarted_Time" - "TaskPickupTime_Start"))/3600 AS "ShelfTime",
    EXTRACT(EPOCH FROM ("CodingTime_Start" - "SprintStarted_Time"))/3600 AS "TaskPickupTime",
...
WHERE
    rank=1 AND
    "SprintStarted_Time" > "TaskPickupTime_Start" AND
    "CodingTime_Start" > "SprintStarted_Time" AND
